...
// 파일명에서 찬양 정보를 추출하는 함수 (다양한 패턴 지원)
...
// 파일명에서 가능한 제목을 추출하는 함수
const extractPossibleTitle = (fileName) => {
  const withoutExt = path.parse(fileName).name;
  ...
  // 너무 짧으면 null 반환
  if (possibleTitle.length < 2) return null;
  
  return possibleTitle;
};

// 첫 가사로 매칭하는 함수
const matchByFirstLyrics = (possibleTitle, songs) => {
  if (!possibleTitle || possibleTitle.length < 2) return [];
...
// 메인 함수
async function renameSheetsEnhanced() {
  try {
    console.log('🎵 향상된 악보 파일명 변경 스크립트 시작...\n');
    ...
    for (const oldFileName of musicFiles) {
      try {
        ...
        // 3. 첫 가사로 매칭 시도
        if (!song) {
          const possibleTitle = extractPossibleTitle(oldFileName);
          if (possibleTitle) {
            const firstLyricsMatches = matchByFirstLyrics(possibleTitle, songsData.songs);
            
            if (firstLyricsMatches.length === 1) {
              song = firstLyricsMatches[0];
              matchMethod = '첫가사';
              console.log(`  🔍 첫 가사로 매칭: ${song.title} (ID: ${song.id}) - "${song.firstLyrics}"`);
            } else if (firstLyricsMatches.length > 1) {
              console.log(`  ⚠️  첫 가사로 여러 찬양이 매칭됩니다. 건너뜀: ${possibleTitle}`);
              continue;
            }
          }
        }
        ...
        if (!song) {
          console.log(`  ⚠️  데이터베이스에서 찬양을 찾을 수 없습니다. 건너뜀.`);
          continue;
        }
        ...
        console.log(`  ✅ ${oldFileName} → ${newFileName} (${matchMethod})`);
        renamedCount++;
    ...
    console.log('🔍 매칭 방식: ID → 제목 → 제목+코드 → 첫가사');
...
if (require.main === module) {
  renameSheetsEnhanced();
}

module.exports = { renameSheetsEnhanced };
